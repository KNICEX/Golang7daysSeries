...
func (s *Session) Insert(values ...any) (int64, error) {
	...
	for _, value := range values {
		// TODO will modify in the future
		s.CallMethod(BeforeInsert, reflect.Indirect(reflect.ValueOf(value)).Addr().Interface())
	...
	for _, value := range values {
		s.CallMethod(AfterInsert, reflect.Indirect(reflect.ValueOf(value)).Addr().Interface())
	}
