...
func (xc *XClient) call(ctx context.Context, rpcAddr string, serviceMethod string, args, reply any) error {
	...
	if ctx == nil {
		...
		return client.CallWithCxt(ctx, serviceMethod, args, reply)
	}
}

func (xc *XClient) Call(serviceMethod string, args, reply any) error {
	rpcAddr, err := xc.d.Get(xc.mode)
	if err != nil {
		return err
	}
	return xc.call(nil, rpcAddr, serviceMethod, args, reply)
}

func (xc *XClient) CallWithCtx(ctx context.Context, serviceMethod string, args, reply any) error {
	rpcAddr, err := xc.d.Get(xc.mode)
	if err != nil {
		return err
	}
